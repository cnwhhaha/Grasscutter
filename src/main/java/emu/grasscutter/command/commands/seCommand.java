...
@Command(
        label = "switchelement",
        usage = "anemo|geo|electro|dendro",
        aliases = {"se"},
        threading = true)
public class seCommand implements CommandHandler {
    ...
    private boolean changeAvatarElement(Player sender, int avatarId, Element element) {
        ...
        AvatarSkillDepotData skillDepot =
                GameData.getAvatarSkillDepotDataMap().get(element.getSkillRepoId(avatarId));
    ...
    public void execute(Player sender, Player targetPlayer, List<String> args) {
        ...
        if (targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getAvatarId()
        ...
        } else if (targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getAvatarId()
                == GameConstants.MAIN_CHARACTER_FEMALE) {
            femaleSuccess =
                    changeAvatarElement(targetPlayer, GameConstants.MAIN_CHARACTER_FEMALE, element);
            ConstellationsHandler.change(targetPlayer, element, constellation);
        }
