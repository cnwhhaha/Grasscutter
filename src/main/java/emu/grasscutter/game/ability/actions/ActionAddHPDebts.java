...
public final class ActionAddHPDebts extends AbilityActionHandler {
    ...
    public boolean execute(
            ...
            GameEntity target) {
        float debt =
                action.ratio.get(ability) * target.getFightProperty(FightProperty.FIGHT_PROP_MAX_HP);
        ...
        if (target instanceof EntityAvatar) {
            ...
            target
                    .getWorld()
                    .broadcastPacket(
                            new PacketEntityFightPropUpdateNotify(target, FightProperty.FIGHT_PROP_CUR_HP_DEBTS));
            if (changeDebt != 0) {
                if (newDebt == 0) {
                    target
                            .getWorld()
                            .broadcastPacket(
                                    new PacketEntityFightPropChangeReasonNotify(
                                            ...
                                            ChangeHpDebtsOuterClass.ChangeHpDebts.CHANGE_HP_DEBTS_PAYFINISH));
                    ...
                    target
                            ...
                            .broadcastPacket(
                                    new PacketEntityFightPropChangeReasonNotify(
                                            ...
                                            changeDebt,
                                            PropChangeReasonOuterClass.PropChangeReason.PROP_CHANGE_REASON_ABILITY,
                                            ChangeHpDebtsOuterClass.ChangeHpDebts.CHANGE_HP_DEBTS_ADDABILiTY));
                } else if (changeDebt < 0) {
                    target
                            .getWorld()
                            .broadcastPacket(
                                    new PacketEntityFightPropChangeReasonNotify(
                                            ...
                                            changeDebt,
                                            PropChangeReasonOuterClass.PropChangeReason.PROP_CHANGE_REASON_ABILITY,
                                            ChangeHpDebtsOuterClass.ChangeHpDebts.CHANGE_HP_DEBTS_PAY));
                }
            }
        } else {
