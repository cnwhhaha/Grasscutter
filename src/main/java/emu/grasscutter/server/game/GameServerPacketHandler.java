...
public final class GameServerPacketHandler {
    ...
    public void handle(GameSession session, int opcode, byte[] header, byte[] payload) {
            ...
            try {
                ...
                if (opcode == PacketOpcodes.PingReq) {
                    ...
                    if (state != SessionState.WAITING_FOR_TOKEN) {
                        Grasscutter.getLogger()
                            .debug("[PH] GetPlayerTokenReq rejected: state=" + state
                                + " expected=WAITING_FOR_TOKEN");
                        return;
                    }
                    ...
                    if (state != SessionState.WAITING_FOR_LOGIN) {
                        Grasscutter.getLogger()
                            .debug("[PH] PlayerLoginReq rejected: state=" + state
                                + " expected=WAITING_FOR_LOGIN");
                        return;
                    }
                } else if (opcode == PacketOpcodes.SetPlayerBornDataReq) {
                    if (state != SessionState.PICKING_CHARACTER) {
                        Grasscutter.getLogger()
                            .debug("[PH] SetPlayerBornDataReq rejected: state=" + state
                                + " expected=PICKING_CHARACTER");
                        return;
                    }
                } else {
                    if (state != SessionState.ACTIVE) {
                        Grasscutter.getLogger()
                            .debug("[PH] Packet " + PacketOpcodesUtils.getOpcodeName(opcode)
                                + " rejected: state=" + state + " expected=ACTIVE");
                        return;
                    }
                }
                ...
                Grasscutter.getLogger()
                    .error("[PH] Handler exception for " + PacketOpcodesUtils.getOpcodeName(opcode)
                        + " (" + opcode + "): " + ex.getMessage(), ex);
            }
        ...
        Grasscutter.getLogger()
            .debug("[PH] Unhandled packet: " + opcode
